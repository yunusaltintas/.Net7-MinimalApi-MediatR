...
namespace Mam.Api.Extensions
{
    ...
    {
        public static WebApplication Get<TRequest>(this WebApplication app, string template) //where TRequest : IRequest
        {
            app.MapGet(template, async (IMediator mediator, [AsParameters] TRequest request) => await mediator.Send(request));
            return app;
        }

        public static WebApplication Post<TRequest>(this WebApplication app, string template) //where TRequest : IRequest
        {
